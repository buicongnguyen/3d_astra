...
export function entryHTML(type) {
  ...
  return `<h2>${d.name}</h2><p>${d.description}</p>
    ...
    ${d.counter ? `<p>Deals ${d.counter_bonus || 1.6}× damage to ${D[d.counter].name}s.</p>` : ''}
    ${d.splash_radius ? `<p>Splash: enemies within ${d.splash_radius} m of the target take ${Math.round(d.splash_damage*100)}% damage.</p>` : ''}
