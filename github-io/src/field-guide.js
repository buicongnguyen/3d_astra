...
export function entryHTML(type) {
  ...
  return `<h2>${d.name}</h2><p>${d.description}</p>
    ...
    ${d.required_level ? `<p>Requires level ${d.required_level} ${type === 'medic' ? 'Barracks' : 'Foundry'}.</p>` : ''}
    ${d.kind === 'building' ? `<h3>Building functions</h3><ul>${(d.trains || []).map(k => `<li>${D[k].name}${D[k].required_level ? ' · level 2' : ''}: ${D[k].cost[0]} alloy / ${D[k].cost[1]} energy</li>`).join('')}${d.supply ? `<li>Provides ${d.supply} supply at level 1.</li>` : ''}${type === 'tower' ? '<li>Automatically attacks visible enemies.</li>' : ''}</ul>
    ...
    <h3>Three technology stages</h3><ol><li>Establish an economy and mixed army.</li><li>Upgrade your core and production to L2; unlock Medics and Engineers.</li><li>Reach L3 for stronger defenses and faster production, then defeat enemy cores.</li></ol><p>These stages occur within either existing skirmish map; they are not separate campaign missions.</p>`;
}
