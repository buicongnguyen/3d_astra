...
export function selectionStatus(sim, entities) {
  ...
  if (!e.complete) status = `Building ${Math.floor(e.progress*100)}%`;
  else if (e.levelJob) status = `Upgrade L${e.level+1} · ${Math.floor(e.levelJob.elapsed/e.levelJob.time*100)}%`;
  else if (!group && e.queue.length) {
    const q=e.queue[0];
    status=`${D[q.type].name} ${Math.min(100,Math.floor(q.elapsed/D[q.type].time*100))}% · ${q.blocked || `Cancel queue ${e.queue.length}/5`}`;
  }
