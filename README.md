...
New players can choose **Training · start here** before starting. Eight safe, guided lessons teach selection, movement, harvesting, construction, production, attack and withdrawal. **Focus target** centers the camera and folds the instruction card; **Leave training** returns to game setup. See the [training implementation plan](TUTORIAL_IMPLEMENTATION_PLAN.md).

Desktop players can use **B** for construction, **Q E R T Y** for the displayed build/train actions, **F2** for army selection, and **H/J/K** for production buildings. Press **?** (or the help button) in the game for clickable commands and control groups, or read the [full shortcut reference](github-io/docs/PC_SHORTCUTS.md).
